...
At the original PR commit `7b865cb`, build `playground-duel-lab-v1`, protocol 7, content `playground-6`, player trace 6, and room replay `duel-lab-1` passed formatting, strict TypeScript, package boundaries, and **78 Bun unit/integration tests (3,150 assertions)**. Rapier initialization/restoration, the 900-tick player replay, the 440-tick duel replay, and the production build passed. The duel replay produced 20 shots, 16 player impacts, four deaths, and four respawns. The existing Rapier bundle-size advisory remains.

Local built-preview and Vite-development Playwright matrices each pass **54/54 cases** across Chromium, Firefox, and WebKit. Both modes exercise elimination, authoritative respawn, and the held-fire release rule at 0, 100, and 200 ms added RTT, alongside existing replay, timing, rendering, admission, and jet coverage. An earlier local preview attempt passed 53/54: Firefox's delayed-reset timing case sampled correction p95 0.2667 units against <0.08. That case passed immediately in isolation and in the unchanged full rerun; keep the intermittent failure as evidence rather than erasing it or relaxing the gate. It was not reproduced in the local development matrix.

**Remote CI failed on PR #1 at `7b865cb`.** Both the [push check](https://github.com/CanLikeTo/derp/actions/runs/35852263127) and [pull-request check](https://github.com/CanLikeTo/derp/actions/runs/35852296304) stopped in the preview Playwright matrix: 31/54 cases passed and 23 failed, so the development matrix did not run. Failures included reset/jet controls and Firefox graphics startup. An [older master run](https://github.com/CanLikeTo/derp/actions/runs/33539679834) shows similar reset/jet and Firefox failures; this suggests at least some CI issues predate the duel slice, but does not establish that every current failure is unrelated. The local green matrices do not supersede the failed remote checks.

The PR-review shield feedback correction passed the full local `check` command with **79 tests (3,154 assertions)** and a production build. Focused shield and duel browser cases passed **12/12 in preview and 12/12 in development** across Chromium, Firefox, and WebKit. The complete 54-case matrices and 30-minute soak above were run before this follow-up; they have not been repeated for the follow-up change.
